...
type state =
  | Unlocked
  | Locked of { fiber : Fiber.Maybe.t; waiters : entry Q.t }
...
let rec cleanup_as entry t backoff =
  ...
  | Locked r as before -> begin
      match r.waiters with
      | T Zero -> unlock_as entry.fiber t backoff
      | T (One _ as q) ->
          let waiters = Q.remove q entry in
          if r.waiters == waiters then unlock_as entry.fiber t backoff
          else
