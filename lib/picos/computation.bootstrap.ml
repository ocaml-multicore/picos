...
and 'a state = S : ('a, [< `Canceled | `Returned | `Continue ]) st -> 'a state
[@@unboxed]
...
module Tx = struct
  ...
  let[@inline] try_cancel (Running _ as tx : [ `Running ] tx) computation exn bt
      =
...
(** [gc] is called when balance becomes negative by both [try_attach] and
    ...
    cause starvation, because the only reason that CAS fails after [gc] is that
    someone else completed the [gc]. *)
