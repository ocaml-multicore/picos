...
module Unix : sig
  (** A transparently asynchronous wrapper for the standard
      {{:https://v2.ocaml.org/api/Unix.html} [Unix]} module.

      All the operations that return file descriptors implicitly set the
      {{!file_descr} file descriptors} to {{!set_nonblock} non-blocking mode}
      and operations on file descriptors, such as {!read} and {!write} and
      others, implicitly block, in a scheduler friendly manner, to await for the
      file descriptor to become available for the operation.

      ℹ️ In this wrapper {{!file_descr} file descriptors} are wrapped to allow
      safe concurrent use internally.  Client code should not use file
      descriptors after {{!close} closing} them.  Typically this means that the
      ownership of file descriptors is managed carefully.

      Please consult the documentation of the
      {{:https://v2.ocaml.org/api/Unix.html} [Unix]} module that comes with
      OCaml. *)

  type file_descr
  (** Represents a wrapped [Unix.file_descr] that stores [Unix.file_descr] and a
      boolean flag indicating whether the file descriptor has been set to
      non-blocking mode. *)

  val unsafe_getfd : file_descr -> Unix.file_descr
  (** [unsafe_getfd fd] returns the underlying [Unix.file_descr] of the wrapped
      {!file_descr}.

      @raise Unix_error with {!EBADF} if the wrapped file descriptor has been
        {{!close} marked as to be closed}.  The underlying file descriptor may
        or may not yet have been closed, but after being {{!close} marked as to
        be closed} one should no longer use the underlying file descriptor to
        start new operations.  This indicates a race condition or logic error in
        your handling of file descriptors.

      @raise Invalid_argument may be raised if the wrapped file descriptor was
        {{!close} closed} concurrently.  This indicates a race condition in your
        handling of file descriptors. *)

  val get_nonblock : file_descr -> bool
  (** [get_nonblock fd] returns [true] if the file descriptor [fd] is set to
      non-blocking mode. *)
  ...
  (** [close file_descr] marks the file descriptor as to be closed.
      ...
      ⚠️ After calling [close] no new operations may be started with the file
      descriptor. *)
