...
module Unix = struct
  ...
  let rec again_0 fd fn op =
    ...
    match fn (Picos_stdio_fd.unsafe_get fd) with
    | result ->
  ...
  let rec again_cloexec_0 ?cloexec fd fn op =
    let intr = intr_req fd in
    match fn ?cloexec (Picos_stdio_fd.unsafe_get fd) with
    | result ->
        Picos_select.Intr.clr intr;
        result
    | exception Unix.Unix_error ((EAGAIN | EINTR | EWOULDBLOCK), _, _) ->
  ...
  let rec again_3 fd x1 x2 x3 fn op =
    let intr = intr_req fd in
    match fn (Picos_stdio_fd.unsafe_get fd) x1 x2 x3 with
    | result ->
        Picos_select.Intr.clr intr;
        result
    | exception Unix.Unix_error ((EAGAIN | EINTR | EWOULDBLOCK), _, _) ->
  ...
  let rec again_4 fd x1 x2 x3 x4 fn op =
    let intr = intr_req fd in
    match fn (Picos_stdio_fd.unsafe_get fd) x1 x2 x3 x4 with
    | result ->
        Picos_select.Intr.clr intr;
        result
    | exception Unix.Unix_error ((EAGAIN | EINTR | EWOULDBLOCK), _, _) ->
  ...
  let rec again_5 fd x1 x2 x3 x4 x5 fn op =
    let intr = intr_req fd in
    match fn (Picos_stdio_fd.unsafe_get fd) x1 x2 x3 x4 x5 with
    | result ->
        Picos_select.Intr.clr intr;
        result
    | exception Unix.Unix_error ((EAGAIN | EINTR | EWOULDBLOCK), _, _) ->
  ...
  let progress_1 fd x1 fn op name =
    let intr = intr_req fd in
    match fn (Picos_stdio_fd.unsafe_get fd) x1 with
    | () -> Picos_select.Intr.clr intr
    | exception
      ...
      begin
        ...
        match Unix.getsockopt_error (Picos_stdio_fd.unsafe_get fd) with
        | None -> ()
  ...
  let stdin = Picos_stdio_fd.create ~dispose:false Unix.stdin
  and stdout = Picos_stdio_fd.create ~dispose:false Unix.stdout
  and stderr = Picos_stdio_fd.create ~dispose:false Unix.stderr

  (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/open.html *)
  let openfile path flags file_perm =
    let fd = Picos_stdio_fd.create (Unix.openfile path flags file_perm) in
    if List.exists is_nonblock flags then begin
      let if_not_added_fd_has_been_closed_outside =
        Picos_htbl.try_add nonblock_fds (Picos_stdio_fd.unsafe_get fd) ()
      in
  ...
  let close fd =
    let _ : bool =
      Picos_htbl.try_remove nonblock_fds (Picos_stdio_fd.unsafe_get fd)
    in
    Picos_stdio_fd.decr ~close:true fd
  ...
  let lseek fd amount seek_command =
    Unix.lseek (Picos_stdio_fd.unsafe_get fd) amount seek_command

  (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/ftruncate.html *)
  let ftruncate fd size = Unix.ftruncate (Picos_stdio_fd.unsafe_get fd) size

  (* *)

  (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/fstat.html *)
  let fstat fd = Unix.fstat (Picos_stdio_fd.unsafe_get fd)

  (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/isatty.html *)
  let isatty fd = Unix.isatty (Picos_stdio_fd.unsafe_get fd)
  ...
  module LargeFile = struct
    ...
    (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/lseek.html *)
    let lseek fd amount seek_command =
      Unix.LargeFile.lseek (Picos_stdio_fd.unsafe_get fd) amount seek_command

    (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/ftruncate.html *)
    let ftruncate fd size =
      Unix.LargeFile.ftruncate (Picos_stdio_fd.unsafe_get fd) size

    (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/fstat.html *)
    let fstat fd = Unix.LargeFile.fstat (Picos_stdio_fd.unsafe_get fd)
  end
  ...
  let map_file fd ?pos kind layout shared dims =
    Unix.map_file (Picos_stdio_fd.unsafe_get fd) ?pos kind layout shared dims

  (* *)

  (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/fchmod.html *)
  let fchmod fd file_perm = Unix.fchmod (Picos_stdio_fd.unsafe_get fd) file_perm

  (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/fchown.html *)
  let fchown fd uid gid = Unix.fchown (Picos_stdio_fd.unsafe_get fd) uid gid

  (* *)

  (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/dup.html *)
  let dup ?cloexec fd =
    Picos_stdio_fd.create (Unix.dup ?cloexec (Picos_stdio_fd.unsafe_get fd))
  ...
  let set_nonblock fd =
    ...
    Picos_htbl.try_add nonblock_fds (Picos_stdio_fd.unsafe_get fd) () |> ignore

  let clear_nonblock fd =
    Unix.clear_nonblock (Picos_stdio_fd.unsafe_get fd);
  ...
  let pipe ?cloexec () =
    ...
    (Picos_stdio_fd.create inn, Picos_stdio_fd.create out)

  (* *)

  let create_process prog args stdin stdout stderr =
    Unix.create_process prog args
      (Picos_stdio_fd.unsafe_get stdin)
      (Picos_stdio_fd.unsafe_get stdout)
      (Picos_stdio_fd.unsafe_get stderr)
  ...
  let lockf fd lock_command length =
    Unix.lockf (Picos_stdio_fd.unsafe_get fd) lock_command length
  ...
  let shutdown fd shutdown_command =
    Unix.shutdown (Picos_stdio_fd.unsafe_get fd) shutdown_command

  (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/getsockname.html *)
  let getsockname fd = Unix.getsockname (Picos_stdio_fd.unsafe_get fd)

  (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/getpeername.html *)
  let getpeername fd = Unix.getpeername (Picos_stdio_fd.unsafe_get fd)
  ...
  let setsockopt fd option bool =
    Unix.setsockopt (Picos_stdio_fd.unsafe_get fd) option bool

  (* https://pubs.opengroup.org/onlinepubs/9699919799/functions/getsockopt.html *)
  let getsockopt_int fd option =
    Unix.getsockopt_int (Picos_stdio_fd.unsafe_get fd) option

  let setsockopt_int fd option int =
    Unix.setsockopt_int (Picos_stdio_fd.unsafe_get fd) option int

  let getsockopt_optint fd option =
    Unix.getsockopt_optint (Picos_stdio_fd.unsafe_get fd) option

  let setsockopt_optint fd option optint =
    Unix.setsockopt_optint (Picos_stdio_fd.unsafe_get fd) option optint

  let getsockopt_float fd option =
    Unix.getsockopt_float (Picos_stdio_fd.unsafe_get fd) option

  let setsockopt_float fd option float =
    Unix.setsockopt_float (Picos_stdio_fd.unsafe_get fd) option float

  let getsockopt_error fd = Unix.getsockopt_error (Picos_stdio_fd.unsafe_get fd)
