module type Default = sig
  module Trigger : sig
    ...
    val await : t -> Picos_exn_bt.t option
  end

  module Computation : sig
    type !'a t

    val cancel_after : 'a t -> seconds:float -> Picos_exn_bt.t -> unit
  end
...
module type Bootstrap = sig
  ...
  module Computation : sig
    ...
    val cancel : 'a t -> Picos_exn_bt.t -> unit
  ...
  module Fiber : sig
    ...
    val canceled : t -> Picos_exn_bt.t option
...
module Make (Bootstrap : Bootstrap) :
  Default
    with type Trigger.t = Bootstrap.Trigger.t
    with type 'a Computation.t = 'a Bootstrap.Computation.t
    with type Fiber.t = Bootstrap.Fiber.t = struct
  ...
  module Trigger = struct
    ...
    let block trigger (per_thread : Per_thread.t) =
      Mutex.lock per_thread.mutex;
      match
        while not (Bootstrap.Trigger.is_signaled trigger) do
  ...
  module Computation = struct
    type 'a t = 'a Bootstrap.Computation.t

    module Entry = struct
      type t =
        | Entry : {
            time : Mtime.span;
            exn_bt : Picos_exn_bt.t;
    ...
    module Per_domain = struct
      ...
      let key =
        Picos_domain.DLS.new_key @@ fun () ->
        {
          ...
          needs_wakeup = true;
          state = `Init;
          exn_bt = Picos_exn_bt.get_callstack 0 Exit;
          pipe_inn = [];
          pipe_out = Select.stdin;
          byte = Bytes.create 1;
        }

      let get () = Picos_domain.DLS.get key
      ...
      let timeout_thread t =
        begin
          try
            ...
            unlock t `Alive;
            timeout_thread_handle t
          with exn -> t.exn_bt <- Picos_exn_bt.get exn
        end;
        t.needs_wakeup <- false;
        unlock t `Dead;
      ...
      let start t =
        ...
        | thread ->
            ...
            if t.exn_bt.exn != Exit then Picos_exn_bt.raise t.exn_bt
        | exception exn ->
            unlock t `Dead;
            raise exn
    ...
    let cancel_after computation ~seconds exn_bt =
      ...
      | Some span ->
          ...
          let remover =
            Bootstrap.Trigger.from_action per_domain id
              Per_domain.remove_timeout
          in
